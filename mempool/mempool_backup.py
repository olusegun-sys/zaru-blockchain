...
class MempoolEntry:
    ...
    """
    fee_per_byte: float = field(compare=True)  # Higher = better (for heapq)
    ...
    # For heapq, we want highest fee_per_byte first (negate for min-heap)
...
class Mempool:
    ...
    def __init__(self, utxo_set=None, store=None, max_size: int = None):
        ...
        self.spent_utxos: Dict[str, Dict[int, str]] = {}  # tx_id:index -> spending_tx_id
    ...
    def get_transactions_for_block(
        ...
        for entry in self.heap:
            ...
            # Remove from mempool (it will be mined)
            # Note: We'll remove all selected transactions after block is mined
            # For now, we just mark them as selected
